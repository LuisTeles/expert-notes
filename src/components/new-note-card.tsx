...
export function NewNoteCard({ onNoteCreated }: NewNoteCardProps) {
  // State for handling whether or not to use the voice recording on a new note
  const [shouldShowOnboarding, setShouldShowOnboarding] = useState(true);
  // State for starting or finishing the recording
  const [isRecording, setIsRecording] = useState(false);
  // State for handling the submit of the content created by the user
  ...
  function handleStartRecording() {
    ...
    // Send a alert if the api is not available
    ...
    // Creating a variable according with the name of the API in each browser
    ...
    // Continue to record until cancel
    speechRecognition.continuous = true;
    // Max alternatives for not recognized words
    speechRecognition.maxAlternatives = 1;
    // Send results during speech not only it ends
        ...
        <Dialog.Content className="fixed overflow-hidden inset-0 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:max-w-[640px] w-full md:h-[60vh] bg-slate-700 md:rounded-md flex flex-col outline-none">
          ...
          <form className="flex flex-1 flex-col">
            ...
            <div className="flex flex-1 flex-col gap-3 p-5">
              ...
              {/* Application of the shouldShowOnboarding state by ternary operator if shouldShowOnboarding is true show the first paragrapher if not show editor */}
